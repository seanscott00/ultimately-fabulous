...
`[[wikilink|wikilink]]` places a [[link]]

`[[link#heading|link > heading]]` links to a heading in a page [[To-Do Lists#Links]]

`[[link^block|link^block]]` links to a block

`![[link|link]]` embeds a link

`[[link|display text]]` changed the display text on the link

`[external link](https://bluecorvid.art/)` is an external link

`[[Fabula_Ultima_TTJRPG-core.pdf#page=70|Fabula_Ultima_TTJRPG-core.pdf > page=70]]` links to a specific page in a pdf
