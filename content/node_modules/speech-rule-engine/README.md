...
**Breaking Change:** Please move to v3.0.2 or later. [More info here.](README.md##breaking-change)
...
1. [**Node Module:**](README.md##node-module) Download via npm or yarn. This is the easiest way to use the speech
rule engine via its API and is the preferred option if you just want to include
it in your project.

2. [**Standalone Tool:**](README.md##standalone-tool) Download via github and build. This is useful
...
3. [**Browser Library:**](README.md##browser-library) This gives you the option of loading
...
| `setupEngine(options)` | Takes an [options feature vector](README.md##options) to parameterise the Speech Rule Engine. Returns a promise that resolves as soon as the engine is ready for processing. |
| `engineSetup()` | Returns the current setup of the engine as an  [options feature vector](README.md##options). |
| `localeLoader()` | SRE's standard method for loading locales, depending on SRE's mode. For more detail see [discussion on locale loading](README.md##locale-loading). |
...
the [Alternative Builds section](README.md##alternative-builds).
