...
- Reads given root recursively and returns a `stream` of [entry infos](README.md##entryinfo)
- Optionally can be used like `for await (const entry of stream)` with node.js 10+ (`asyncIterator`).
- `on('data', (entry) => {})` [entry info](README.md.md##entryinfo) for every file / dir.
...
`const entries = await readdirp.promise(root[, options])` — **Promise API**. Returns a list of [entry infos](README.md##entryinfo).
...
MIT License, see [LICENSE](../ms/license.md#) file.
