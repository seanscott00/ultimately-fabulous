...
* [Iterables](README.md##iterables)
* [Sets](README.md##sets)
* [Bins](README.md##bins)
* [Interning](README.md##interning)
...
See also [extent](README.md##extent).
...
Returns the [minimum](README.md##min) and [maximum](README.md##max) value in the given *iterable* using natural order. If the iterable contains no comparable values, returns [undefined, undefined]. An optional *accessor* function may be specified, which is equivalent to calling Array.from before computing the extent.
...
Returns an array with the rank of each value in the *iterable*, *i.e.* the zero-based index of the value when the iterable is sorted. Nullish values are sorted to the end and ranked NaN. An optional *comparator* or *accessor* function may be specified; the latter is equivalent to calling *array*.map(*accessor*) before computing the ranks. If *comparator* is not specified, it defaults to [ascending](README.md##ascending). Ties (equivalent values) all get the same rank, defined as the first time the value is found.
...
Returns the standard deviation, defined as the square root of the [bias-corrected variance](README.md##variance), of the given *iterable* of numbers. If the iterable has fewer than two numbers, returns undefined. An optional *accessor* function may be specified, which is equivalent to calling Array.from before computing the standard deviation. This method ignores undefined and NaN values; this is useful for ignoring missing data.
...
This function is similar to [min](README.md.md##min), except it allows the use of a comparator rather than an accessor.
...
This function is similar to [minIndex](README.md##minIndex), except it allows the use of a comparator rather than an accessor.
...
This function is similar to [max](README.md.md##max), except it allows the use of a comparator rather than an accessor.
...
This function is similar to [maxIndex](README.md##maxIndex), except it allows the use of a comparator rather than an accessor.
...
Similar to [bisectLeft](README.md##bisectLeft), but returns an insertion point which comes after (to the right of) any existing entries of *x* in *array*. The returned insertion point *i* partitions the *array* into two halves so that all *v* <= *x* for *v* in *array*.slice(*lo*, *i*) for the left side and all *v* > *x* for *v* in *array*.slice(*i*, *hi*) for the right side.
...
See [*bisector*.center](README.md##bisector_center).
...
Equivalent to [bisectLeft](README.md##bisectLeft), but uses this bisector’s associated comparator.
...
Equivalent to [bisectRight](README.md##bisectRight), but uses this bisector’s associated comparator.
...
Groups the specified *iterable* of values into an [InternMap](README.md##InternMap) from *key* to array of value. For example, given some data:
...
Equivalent to [group](README.md##group), but returns nested arrays instead of nested maps.
...
Equivalent to [group](README.md##group), but returns a flat array of [*key0*, *key1*, …, *values*] instead of nested maps.
...
Equivalent to [group](README.md##group) but returns a unique value per compound key instead of an array, throwing if the key is not unique.
...
Equivalent to [index](README.md##index), but returns nested arrays instead of nested maps.
...
[Groups](README.md.md.md.md##group) and reduces the specified *iterable* of values into an InternMap from *key* to value. For example, given some data:
...
To convert a Map to an Array, use [Array.from](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/from). See [d3.group](README.md.md.md.md.md##group) for examples.
...
Equivalent to [rollup](README.md##rollup), but returns nested arrays instead of nested maps.
...
Equivalent to [rollup](README.md##rollup), but returns a flat array of [*key0*, *key1*, …, *value*] instead of nested maps.
...
Returns a [shuffle function](README.md##shuffle) given the specified random source. For example, using [d3.randomLcg](https://github.com/d3/d3-random/blob/main/README.md#randomLcg):
...
Returns an array of approximately *count* + 1 uniformly-spaced, nicely-rounded values between *start* and *stop* (inclusive). Each value is a power of ten multiplied by 1, 2 or 5. See also [d3.tickIncrement](README.md##tickIncrement), [d3.tickStep](README.md##tickStep) and [*linear*.ticks](https://github.com/d3/d3-scale/blob/main/README.md#linear_ticks).

Ticks are inclusive in the sense that they may include the specified *start* and *stop* values if (and only if) they are exact, nicely-rounded values consistent with the inferred [step](README.md.md##tickStep). More formally, each returned tick *t* satisfies *start* ≤ *t* and *t* ≤ *stop*.
...
Like [d3.tickStep](README.md.md.md##tickStep), except requires that *start* is always less than or equal to *stop*, and if the tick step for the given *start*, *stop* and *count* would be less than one, returns the negative inverse tick step instead. This method is always guaranteed to return an integer, and is used by [d3.ticks](README.md##ticks) to guarantee that the returned tick values are represented as precisely as possible in IEEE 754 floating point.
...
Returns a new interval [*niceStart*, *niceStop*] covering the given interval [*start*, *stop*] and where *niceStart* and *niceStop* are guaranteed to align with the corresponding [tick step](README.md.md.md.md##tickStep). Like [d3.tickIncrement](README.md##tickIncrement), this requires that *start* is less than or equal to *stop*.
...
Returns an array containing an arithmetic progression, similar to the Python built-in [range](http://docs.python.org/library/functions.html#range). This method is often used to iterate over a sequence of uniformly-spaced numeric values, such as the indexes of an array or the ticks of a linear scale. (See also [d3.ticks](README.md.md##ticks) for nicely-rounded values.)
...
Uses the [zip](README.md##zip) operator as a two-dimensional [matrix transpose](http://en.wikipedia.org/wiki/Transpose).
...
Returns an array containing the values in the given *iterable* in the sorted order defined by the given *comparator* or *accessor* function. If *comparator* is not specified, it defaults to [d3.ascending](README.md.md.md.md.md.md##ascending). Equivalent to [*array*.sort](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort), except that it does not mutate the given *iterable*, and the comparator defaults to natural order instead of lexicographic order:
...
it is equivalent to a *comparator* using [natural order](README.md.md.md.md.md.md.md##ascending):
...
Any null or non-comparable values in the given *data*, or those outside the [domain](README.md##bin_domain), are ignored.
...
If *domain* is specified, sets the domain accessor to the specified function or array and returns this bin generator. If *domain* is not specified, returns the current domain accessor, which defaults to [extent](README.md##extent). The bin domain is defined as an array [*min*, *max*], where *min* is the minimum observable value and *max* is the maximum observable value; both values are inclusive. Any value outside of this domain will be ignored when the bins are [generated](README.md##_bin).
...
If the default [extent](README.md##extent) domain is used and the [thresholds](README.md##bin_thresholds) are specified as a count (rather than explicit values), then the computed domain will be [niced](README.md##nice) such that all bins are uniform width.

Note that the domain accessor is invoked on the materialized array of [values](README.md##bin_value), not on the input data array.
...
If *thresholds* is specified, sets the [threshold generator](README.md##bin-thresholds) to the specified function or array and returns this bin generator. If *thresholds* is not specified, returns the current threshold generator, which by default implements [Sturges’ formula](README.md##thresholdSturges). (Thus by default, the values to be binned must be numbers!) Thresholds are defined as an array of values [*x0*, *x1*, …]. Any value less than *x0* will be placed in the first bin; any value greater than or equal to *x0* but less than *x1* will be placed in the second bin; and so on. Thus, the [generated bins](README.md.md.md##_bin) will have *thresholds*.length + 1 bins. See [bin thresholds](README.md.md##bin-thresholds) for more information.

Any threshold values outside the [domain](README.md##bin_domain) are ignored. The first *bin*.x0 is always equal to the minimum domain value, and the last *bin*.x1 is always equal to the maximum domain value.

If a *count* is specified instead of an array of *thresholds*, then the [domain](README.md##bin_domain) will be uniformly divided into approximately *count* bins; see [ticks](README.md.md.md.md##ticks).

#### Bin Thresholds

These functions are typically not used directly; instead, pass them to [*bin*.thresholds](README.md.md##bin_thresholds).
...
You may also implement your own threshold generator taking three arguments: the array of input [*values*](README.md.md##bin_value) derived from the data, and the [observable domain](README.md.md.md.md##bin_domain) represented as *min* and *max*. The generator may then return either the array of numeric thresholds or the *count* of bins; in the latter case the domain is divided uniformly into approximately *count* bins; see [ticks](README.md.md.md.md.md##ticks).
