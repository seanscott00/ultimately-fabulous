...
To apply a transition, select elements, call [*selection*.transition](README.md##selection_transition), and then make the desired changes. For example:
...
Transitions support most selection methods (such as [*transition*.attr](README.md##transition_attr) and [*transition*.style](README.md##transition_style) in place of [*selection*.attr](https://github.com/d3/d3-selection#selection_attr) and [*selection*.style](https://github.com/d3/d3-selection#selection_style)), but not all methods are supported; for example, you must [append](https://github.com/d3/d3-selection#selection_append) elements or [bind data](https://github.com/d3/d3-selection#joining-data) before a transition starts. A [*transition*.remove](README.md##transition_remove) operator is provided for convenient removal of elements when the transition ends.

To compute intermediate state, transitions leverage a variety of [built-in interpolators](https://github.com/d3/d3-interpolate). [Colors](https://github.com/d3/d3-interpolate#interpolateRgb), [numbers](https://github.com/d3/d3-interpolate#interpolateNumber), and [transforms](https://github.com/d3/d3-interpolate#interpolateTransform) are automatically detected. [Strings](https://github.com/d3/d3-interpolate#interpolateString) with embedded numbers are also detected, as is common with many styles (such as padding or font sizes) and paths. To specify a custom interpolator, use [*transition*.attrTween](README.md##transition_attrTween), [*transition*.styleTween](README.md##transition_styleTween) or [*transition*.tween](README.md##transition_tween).
...
* [The Life of a Transition](README.md##the-life-of-a-transition)

### Selecting Elements

Transitions are derived from [selections](https://github.com/d3/d3-selection) via [*selection*.transition](README.md##selection_transition). You can also create a transition on the document root element using [d3.transition](README.md##transition).
...
Interrupts the active transition of the specified *name* on the specified *node*, and cancels any pending transitions with the specified *name*, if any. If a name is not specified, null is used. See also [*selection*.interrupt](README.md##selection_interrupt).
...
This method is equivalent to deriving the selection for this transition via [*transition*.selection](README.md##transition_selection), creating a subselection via [*selection*.select](https://github.com/d3/d3-selection#selection_select), and then creating a new transition via [*selection*.transition](README.md##selection_transition):
...
This method is equivalent to deriving the selection for this transition via [*transition*.selection](README.md##transition_selection), creating a subselection via [*selection*.selectAll](https://github.com/d3/d3-selection#selection_selectAll), and then creating a new transition via [*selection*.transition](README.md##selection_transition):
...
This method is equivalent to deriving the selection for this transition via [*transition*.selection](README.md##transition_selection), creating a subselection via [*selection*.selectChild](https://github.com/d3/d3-selection#selection_selectChild), and then creating a new transition via [*selection*.transition](README.md##selection_transition):
...
This method is equivalent to deriving the selection for this transition via [*transition*.selection](README.md##transition_selection), creating a subselection via [*selection*.selectChildren](https://github.com/d3/d3-selection#selection_selectChildren), and then creating a new transition via [*selection*.transition](README.md##selection_transition):
...
This method is equivalent to deriving the selection for this transition via [*transition*.selection](README.md##transition_selection), creating a subselection via [*selection*.filter](https://github.com/d3/d3-selection#selection_filter), and then creating a new transition via [*selection*.transition](README.md##selection_transition):
...
This method is equivalent to deriving the selection for this transition via [*transition*.selection](README.md##transition_selection), merging with the selection likewise derived from the *other* transition via [*selection*.merge](https://github.com/d3/d3-selection#selection_merge), and then creating a new transition via [*selection*.transition](README.md##selection_transition):
...
Returns a new transition on the same selected elements as this transition, scheduled to start when this transition ends. The new transition inherits a reference time equal to this transition’s time plus its [delay](README.md##transition_delay) and [duration](README.md##transition_duration). The new transition also inherits this transition’s name, duration, and [easing](README.md##transition_ease). This method can be used to schedule a sequence of chained transitions. For example:
...
After selecting elements and creating a transition with [*selection*.transition](README.md##selection_transition), use the transition’s transformation methods to affect document content.
...
To apply a different interpolator, use [*transition*.attrTween](README.md.md.md##transition_attrTween).
...
If *factory* is specified and not null, assigns the attribute [tween](README.md.md##transition_tween) for the attribute with the specified *name* to the specified interpolator *factory*. An interpolator factory is a function that returns an [interpolator](https://github.com/d3/d3-interpolate); when the transition starts, the *factory* is evaluated for each selected element, in order, being passed the current datum (*d*), the current index (*i*), and the current group (*nodes*), with *this* as the current DOM element. The returned interpolator will then be invoked for each frame of the transition, in order, being passed the [eased](README.md.md##transition_ease) time *t*, typically in the range [0, 1]. Lastly, the return value of the interpolator will be used to set the attribute value. The interpolator must return a string. (To remove an attribute at the start of a transition, use [*transition*.attr](README.md##transition_attr); to remove an attribute at the end of a transition, use [*transition*.on](README.md##transition_on) to listen for the *end* event.)
...
Or to interpolate from the current fill to blue, like [*transition*.attr](README.md##transition_attr):
...
To apply a different interpolator, use [*transition*.styleTween](README.md.md.md##transition_styleTween).
...
If *factory* is specified and not null, assigns the style [tween](README.md.md##transition_tween) for the style with the specified *name* to the specified interpolator *factory*. An interpolator factory is a function that returns an [interpolator](https://github.com/d3/d3-interpolate); when the transition starts, the *factory* is evaluated for each selected element, in order, being passed the current datum (*d*), the current index (*i*), and the current group (*nodes*), with *this* as the current DOM element. The returned interpolator will then be invoked for each frame of the transition, in order, being passed the [eased](README.md.md.md##transition_ease) time *t*, typically in the range [0, 1]. Lastly, the return value of the interpolator will be used to set the style value with the specified *priority*. The interpolator must return a string. (To remove an style at the start of a transition, use [*transition*.style](README.md.md##transition_style); to remove an style at the end of a transition, use [*transition*.on](README.md.md##transition_on) to listen for the *end* event.)
...
Or to interpolate from the current fill to blue, like [*transition*.style](README.md.md.md##transition_style):
...
To interpolate text rather than to set it on start, use [*transition*.textTween](README.md##transition_textTween) or append a replacement element and cross-fade opacity. Text is not interpolated by default because it is usually undesirable.
...
If *factory* is specified and not null, assigns the text [tween](README.md.md##transition_tween) to the specified interpolator *factory*. An interpolator factory is a function that returns an [interpolator](https://github.com/d3/d3-interpolate); when the transition starts, the *factory* is evaluated for each selected element, in order, being passed the current datum `d` and index `i`, with the `this` context as the current DOM element. The returned interpolator will then be invoked for each frame of the transition, in order, being passed the [eased](README.md.md.md.md.md##transition_ease) time *t*, typically in the range [0, 1]. Lastly, the return value of the interpolator will be used to set the text. The interpolator must return a string.
...
The [easing](README.md.md.md.md.md.md##transition_ease), [delay](README.md##transition_delay) and [duration](README.md##transition_duration) of a transition is configurable. For example, a per-element delay can be used to [stagger the reordering](https://observablehq.com/@d3/sortable-bar-chart) of elements, improving perception. See [Animated Transitions in Statistical Data Graphics](http://vis.berkeley.edu/papers/animated_transitions/) for recommendations.
...
See [The Life of a Transition](README.md##the-life-of-a-transition) for more. Note that these are *not* native DOM events as implemented by [*selection*.on](https://github.com/d3/d3-selection#selection_on) and [*selection*.dispatch](https://github.com/d3/d3-selection#selection_dispatch), but transition events!
...
Immediately after creating a transition, such as by [*selection*.transition](README.md##selection_transition) or [*transition*.transition](README.md##transition_transition), you may configure the transition using methods such as [*transition*.delay](README.md.md.md##transition_delay), [*transition*.duration](README.md.md.md##transition_duration), [*transition*.attr](README.md##transition_attr) and [*transition*.style](README.md##transition_style). Methods that specify target values (such as *transition*.attr) are evaluated synchronously; however, methods that require the starting value for interpolation, such as [*transition*.attrTween](README.md.md.md##transition_attrTween) and [*transition*.styleTween](README.md.md.md##transition_styleTween), must be deferred until the transition starts.
...
For each frame that a transition is active, it invokes its tweens with an [eased](README.md.md.md.md.md##transition_ease) *t*-value ranging from 0 to 1. Within each frame, the transition invokes its tweens in the order they were registered.
