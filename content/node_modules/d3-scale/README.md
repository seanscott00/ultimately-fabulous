...
For [continuous](README.md##continuous-scales) quantitative data, you typically want a [linear scale](README.md##linear-scales). (For time series data, a [time scale](README.md##time-scales).) If the distribution calls for it, consider transforming data using a [power](README.md##power-scales) or [log](README.md##log-scales) scale. A [quantize scale](README.md##quantize-scales) may aid differentiation by rounding continuous data to a fixed set of discrete values; similarly, a [quantile scale](README.md##quantile-scales) computes quantiles from a sample population, and a [threshold scale](README.md##threshold-scales) allows you to specify arbitrary breaks in continuous data.

For discrete ordinal (ordered) or categorical (unordered) data, an [ordinal scale](README.md##ordinal-scales) specifies an explicit mapping from a set of data values to a corresponding set of visual attributes (such as colors). The related [band](README.md##band-scales) and [point](README.md##point-scales) scales are useful for position-encoding ordinal data, such as bars in a bar chart or dots in an categorical scatterplot.
...
Scales have no intrinsic visual representation. However, most scales can [generate](README.md##continuous_ticks) and [format](README.md##continuous_tickFormat) ticks for reference marks to aid in the construction of axes.
...
(You can omit d3-time and d3-time-format if you’re not using [d3.scaleTime](README.md##scaleTime) or [d3.scaleUtc](README.md##scaleUtc).)

## API Reference

* [Continuous](README.md.md##continuous-scales) ([Linear](README.md.md##linear-scales), [Power](README.md.md##power-scales), [Log](README.md.md##log-scales), [Identity](README.md##identity-scales), [Time](README.md.md##time-scales), [Radial](README.md##radial-scales))
* [Sequential](README.md##sequential-scales)
* [Diverging](README.md##diverging-scales)
* [Quantize](README.md.md##quantize-scales)
* [Quantile](README.md.md##quantile-scales)
* [Threshold](README.md.md##threshold-scales)
* [Ordinal](README.md.md##ordinal-scales) ([Band](README.md.md##band-scales), [Point](README.md.md##point-scales))

### Continuous Scales

Continuous scales map a continuous, quantitative input [domain](README.md##continuous_domain) to a continuous output [range](README.md##continuous_range). If the range is also numeric, the mapping may be [inverted](README.md##continuous_invert). A continuous scale is not constructed directly; instead, try a [linear](README.md.md.md##linear-scales), [power](README.md.md.md##power-scales), [log](README.md.md.md##log-scales), [identity](README.md.md##identity-scales), [radial](README.md.md##radial-scales), [time](README.md.md.md##time-scales) or [sequential color](README.md.md##sequential-scales) scale.
...
Given a *value* from the [domain](README.md##continuous_domain), returns the corresponding value from the [range](README.md.md##continuous_range). If the given *value* is outside the domain, and [clamping](README.md##continuous_clamp) is not enabled, the mapping may be extrapolated such that the returned value is outside the range. For example, to apply a position encoding:
...
Given a *value* from the [range](README.md.md.md##continuous_range), returns the corresponding value from the [domain](README.md##continuous_domain). Inversion is useful for interaction, say to determine the data value corresponding to the position of the mouse. For example, to invert a position encoding:
...
If the given *value* is outside the range, and [clamping](README.md.md##continuous_clamp) is not enabled, the mapping may be extrapolated such that the returned value is outside the domain. This method is only supported if the range is numeric. If the range is not numeric, returns NaN.
...
If *range* is specified, sets the scale’s range to the specified array of values. The array must contain two or more elements. Unlike the [domain](README.md##continuous_domain), elements in the given array need not be numbers; any value that is supported by the underlying [interpolator](README.md##continuous_interpolate) will work, though note that numeric ranges are required for [invert](README.md.md##continuous_invert). If *range* is not specified, returns a copy of the scale’s current range. See [*continuous*.interpolate](README.md.md##continuous_interpolate) for more examples.
...
Sets the scale’s [*range*](README.md.md.md.md##continuous_range) to the specified array of values while also setting the scale’s [interpolator](README.md.md.md##continuous_interpolate) to [interpolateRound](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolateRound). This is a convenience method equivalent to:
...
If *clamp* is specified, enables or disables clamping accordingly. If clamping is disabled and the scale is passed a value outside the [domain](README.md##continuous_domain), the scale may return a value outside the [range](README.md.md.md##continuous_range) through extrapolation. If clamping is enabled, the return value of the scale is always within the scale’s range. Clamping similarly applies to [*continuous*.invert](README.md.md.md##continuous_invert). For example:
...
Returns approximately *count* representative values from the scale’s [domain](README.md##continuous_domain). If *count* is not specified, it defaults to 10. The returned tick values are uniformly spaced, have human-readable values (such as multiples of powers of 10), and are guaranteed to be within the extent of the domain. Ticks are often used to display reference lines, or tick marks, in conjunction with the visualized data. The specified *count* is only a hint; the scale may return more or fewer values depending on the domain. See also d3-array’s [ticks](https://github.com/d3/d3-array/blob/master/README.md#ticks).
...
See also [d3.tickFormat](README.md##tickFormat).
...
Extends the [domain](README.md##continuous_domain) so that it starts and ends on nice round values. This method typically modifies the scale’s domain, and may only extend the bounds to the nearest round value. An optional tick *count* argument allows greater control over the step size used to extend the bounds, guaranteeing that the returned [ticks](README.md.md.md##continuous_ticks) will exactly cover the domain. Nicing is useful if the domain is computed from data, say using [extent](https://github.com/d3/d3-array/blob/master/README.md#extent), and may be irregular. For example, for a domain of [0.201479…, 0.996679…], a nice domain might be [0.2, 1.0]. If the domain has more than two values, nicing the domain only affects the first and last value. See also d3-array’s [tickStep](https://github.com/d3/d3-array/blob/master/README.md#tickStep).

Nicing a scale only modifies the current domain; it does not automatically nice domains that are subsequently set using [*continuous*.domain](README.md.md.md.md.md.md.md.md##continuous_domain). You must re-nice the scale after setting the new domain, if desired.
...
Constructs a new [continuous scale](README.md.md.md##continuous-scales) with the specified [domain](README.md##continuous_domain) and [range](README.md.md.md##continuous_range), the [default](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolate) [interpolator](README.md.md.md.md##continuous_interpolate) and [clamping](README.md.md##continuous_clamp) disabled. If either *domain* or *range* are not specified, each defaults to [0, 1]. Linear scales are a good default choice for continuous quantitative data because they preserve proportional differences. Each range value *y* can be expressed as a function of the domain value *x*: *y* = *mx* + *b*.

#### Power Scales

Power scales are similar to [linear scales](README.md.md.md.md##linear-scales), except an exponential transform is applied to the input domain value before the output range value is computed. Each range value *y* can be expressed as a function of the domain value *x*: *y* = *mx^k* + *b*, where *k* is the [exponent](README.md##pow_exponent) value. Power scales also support negative domain values, in which case the input value and the resulting output value are multiplied by -1.
...
Constructs a new [continuous scale](README.md.md.md##continuous-scales) with the specified [domain](README.md##continuous_domain) and [range](README.md.md.md##continuous_range), the [exponent](README.md.md##pow_exponent) 1, the [default](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolate) [interpolator](README.md.md.md.md.md##continuous_interpolate) and [clamping](README.md.md##continuous_clamp) disabled. If either *domain* or *range* are not specified, each defaults to [0, 1]. (Note that this is effectively a [linear](README.md.md.md.md.md##linear-scales) scale until you set a different exponent.)
...
See [*continuous*](README.md##_continuous).
...
See [*continuous*.invert](README.md.md.md.md##continuous_invert).
...
If *exponent* is specified, sets the current exponent to the given numeric value. If *exponent* is not specified, returns the current exponent, which defaults to 1. (Note that this is effectively a [linear](README.md.md.md.md.md.md##linear-scales) scale until you set a different exponent.)
...
See [*continuous*.domain](README.md.md.md.md.md.md.md.md##continuous_domain).
...
See [*continuous*.range](README.md.md.md.md.md.md.md.md.md##continuous_range).
...
See [*continuous*.rangeRound](README.md##continuous_rangeRound).
...
See [*continuous*.clamp](README.md.md.md.md.md##continuous_clamp).
...
See [*continuous*.interpolate](README.md.md.md.md.md.md##continuous_interpolate).
...
See [*continuous*.ticks](README.md.md.md.md##continuous_ticks).
...
See [*continuous*.tickFormat](README.md.md##continuous_tickFormat).
...
See [*continuous*.nice](README.md##continuous_nice).
...
See [*continuous*.copy](README.md##continuous_copy).
...
Constructs a new [continuous](README.md##continuous-scales) [power scale](README.md.md.md.md##power-scales) with the specified [domain](README.md##continuous_domain) and [range](README.md.md.md##continuous_range), the [exponent](README.md.md.md##pow_exponent) 0.5, the [default](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolate) [interpolator](README.md.md.md.md.md.md.md##continuous_interpolate) and [clamping](README.md.md##continuous_clamp) disabled. If either *domain* or *range* are not specified, each defaults to [0, 1]. This is a convenience method equivalent to `d3.scalePow(…).exponent(0.5)`.

#### Log Scales

Log scales are similar to [linear scales](README.md.md.md.md##linear-scales), except a logarithmic transform is applied to the input domain value before the output range value is computed. The mapping to the range value *y* can be expressed as a function of the domain value *x*: *y* = *m* log(<i>x</i>) + *b*.
...
Constructs a new [continuous scale](README.md.md.md##continuous-scales) with the specified [domain](README.md##log_domain) and [range](README.md##log_range), the [base](README.md##log_base) 10, the [default](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolate) [interpolator](README.md##log_interpolate) and [clamping](README.md##log_clamp) disabled. If *domain* is not specified, it defaults to [1, 10]. If *range* is not specified, it defaults to [0, 1].
...
Like [*continuous*.ticks](README.md.md.md.md##continuous_ticks), but customized for a log scale. If the [base](README.md.md##log_base) is an integer, the returned ticks are uniformly spaced within each integer power of base; otherwise, one tick per power of base is returned. The returned ticks are guaranteed to be within the extent of the domain. If the orders of magnitude in the [domain](README.md.md##log_domain) is greater than *count*, then at most one tick per power is returned. Otherwise, the tick values are unfiltered, but note that you can use [*log*.tickFormat](README.md##log_tickFormat) to filter the display of tick labels. If *count* is not specified, it defaults to 10.
...
Like [*continuous*.nice](README.md##continuous_nice), except extends the domain to integer powers of [base](README.md.md.md##log_base). For example, for a domain of [0.201479…, 0.996679…], and base 10, the nice domain is [0.1, 1]. If the domain has more than two values, nicing the domain only affects the first and last value.
...
Constructs a new [continuous scale](README.md.md.md##continuous-scales) with the specified [domain](README.md##continuous_domain) and [range](README.md.md.md##continuous_range), the [constant](README.md##symlog_constant) 1, the [default](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolate) [interpolator](README.md.md.md.md.md.md.md.md.md##continuous_interpolate) and [clamping](README.md.md##continuous_clamp) disabled. If *domain* is not specified, it defaults to [0, 1]. If *range* is not specified, it defaults to [0, 1].
...
Identity scales are a special case of [linear scales](README.md.md.md.md##linear-scales) where the domain and range are identical; the scale and its invert method are thus the identity function. These scales are occasionally useful when working with pixel coordinates, say in conjunction with an axis. Identity scales do not support [rangeRound](README.md.md.md##continuous_rangeRound), [clamp](README.md.md.md.md.md.md.md.md.md##continuous_clamp) or [interpolate](README.md.md.md.md.md.md.md.md.md.md##continuous_interpolate).
...
Constructs a new identity scale with the specified [domain](README.md##continuous_domain) and [range](README.md.md.md##continuous_range). If *range* is not specified, it defaults to [0, 1].

#### Radial Scales

Radial scales are a variant of [linear scales](README.md.md.md.md##linear-scales) where the range is internally squared so that an input value corresponds linearly to the squared output value. These scales are useful when you want the input value to correspond to the area of a graphical mark and the mark is specified by radius, as in a radial bar chart. Radial scales do not support [interpolate](README.md.md.md.md.md.md.md.md.md.md.md##continuous_interpolate).
...
Constructs a new radial scale with the specified [domain](README.md##continuous_domain) and [range](README.md.md.md##continuous_range). If *domain* or *range* is not specified, each defaults to [0, 1].

#### Time Scales

Time scales are a variant of [linear scales](README.md.md.md.md##linear-scales) that have a temporal domain: domain values are coerced to [dates](https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Date) rather than numbers, and [invert](README.md.md.md.md.md.md##continuous_invert) likewise returns a date. Time scales implement [ticks](README.md##time_ticks) based on [calendar intervals](https://github.com/d3/d3-time), taking the pain out of generating axes for temporal domains.
...
Constructs a new time scale with the specified [domain](README.md##time_domain) and [range](README.md##time_range), the [default](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolate) [interpolator](README.md##time_interpolate) and [clamping](README.md##time_clamp) disabled. If *domain* is not specified, it defaults to [2000-01-01, 2000-01-02]. If *range* is not specified, it defaults to [0, 1].
...
Returns representative dates from the scale’s [domain](README.md##time_domain). The returned tick values are uniformly-spaced (mostly), have sensible values (such as every day at midnight), and are guaranteed to be within the extent of the domain. Ticks are often used to display reference lines, or tick marks, in conjunction with the visualized data.
...
Returns a time format function suitable for displaying [tick](README.md.md##time_ticks) values. The specified *count* or *interval* is currently ignored, but is accepted for consistency with other scales such as [*continuous*.tickFormat](README.md.md##continuous_tickFormat). If a format *specifier* is specified, this method is equivalent to [format](https://github.com/d3/d3-time-format/blob/master/README.md#format). If *specifier* is not specified, the default time format is returned. The default multi-scale time format chooses a human-readable representation based on the specified date as follows:
...
Extends the [domain](README.md##time_domain) so that it starts and ends on nice round values. This method typically modifies the scale’s domain, and may only extend the bounds to the nearest round value. See [*continuous*.nice](README.md##continuous_nice) for more.

An optional tick *count* argument allows greater control over the step size used to extend the bounds, guaranteeing that the returned [ticks](README.md.md.md##time_ticks) will exactly cover the domain. Alternatively, a [time *interval*](https://github.com/d3/d3-time/blob/master/README.md#intervals) may be specified to explicitly set the ticks. If an *interval* is specified, an optional *step* may also be specified to skip some ticks. For example, `time.nice(d3.timeSecond.every(10))` will extend the domain to an even ten seconds (0, 10, 20, <i>etc.</i>). See [*time*.ticks](#time_ticks) and [*interval*.every](https://github.com/d3/d3-time/blob/master/README.md#interval_every) for further detail.
...
Equivalent to [scaleTime](README.md.md##scaleTime), but the returned time scale operates in [Coordinated Universal Time](https://en.wikipedia.org/wiki/Coordinated_Universal_Time) rather than local time.

### Sequential Scales

Sequential scales, like [diverging scales](README.md.md.md##diverging-scales), are similar to [continuous scales](README.md.md.md.md.md.md.md.md##continuous-scales) in that they map a continuous, numeric input domain to a continuous output range. However, unlike continuous scales, the input domain and output range of a sequential scale always has exactly two elements, and the output range is typically specified as an interpolator rather than an array of values. These scales do not expose [invert](README.md.md.md.md.md.md.md.md##continuous_invert) and [interpolate](README.md.md.md.md.md.md.md.md.md.md.md.md.md##continuous_interpolate) methods.
...
Constructs a new sequential scale with the specified [*domain*](README.md##sequential_domain) and [*interpolator*](README.md##sequential_interpolator) function or array. If *domain* is not specified, it defaults to [0, 1]. If *interpolator* is not specified, it defaults to the identity function. When the scale is [applied](README.md##_sequential), the interpolator will be invoked with a value typically in the range [0, 1], where 0 represents the minimum value and 1 represents the maximum value. For example, to implement the ill-advised [HSL](https://github.com/d3/d3-color/blob/master/README.md#hsl) rainbow scale:
...
See [*continuous*.domain](README.md.md.md.md.md.md.md.md##continuous_domain). Note that a sequential scale’s domain must be numeric and must contain exactly two values.
...
See [*continuous*.range](README.md.md.md.md.md.md.md.md.md##continuous_range). If *range* is specified, the given two-element array is converted to an interpolator function using [d3.interpolate](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolate).
...
See [*continuous*.rangeRound](README.md##continuous_rangeRound). If *range* is specified, implicitly uses [d3.interpolateRound](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolateRound) as the interpolator.
...
A [sequential scale](README.md.md.md##sequential-scales) with a logarithmic transform, analogous to a [log scale](README.md.md.md.md##log-scales).
...
A [sequential scale](README.md.md.md##sequential-scales) with an exponential transform, analogous to a [power scale](README.md##pow-scales).
...
A [sequential scale](README.md.md.md##sequential-scales) with a square-root transform, analogous to a [d3.scaleSqrt](README.md##scaleSqrt).
...
A [sequential scale](README.md.md.md##sequential-scales) with a symmetric logarithmic transform, analogous to a [symlog scale](README.md##symlog-scales).
...
A [sequential scale](README.md.md.md##sequential-scales) using a *p*-quantile transform, analogous to a [quantile scale](README.md.md.md##quantile-scales).
...
Diverging scales, like [sequential scales](README.md.md.md.md.md.md.md.md##sequential-scales), are similar to [continuous scales](README.md.md.md.md.md.md.md.md.md##continuous-scales) in that they map a continuous, numeric input domain to a continuous output range. However, unlike continuous scales, the input domain and output range of a diverging scale always has exactly three elements, and the output range is typically specified as an interpolator rather than an array of values. These scales do not expose [invert](README.md.md.md.md.md.md.md.md.md##continuous_invert) and [interpolate](README.md.md.md.md.md.md.md.md.md.md.md.md.md.md##continuous_interpolate) methods.
...
Constructs a new diverging scale with the specified [*domain*](README.md##diverging_domain) and [*interpolator*](README.md##diverging_interpolator) function or array. If *domain* is not specified, it defaults to [0, 0.5, 1]. If *interpolator* is not specified, it defaults to the identity function. When the scale is [applied](README.md##_diverging), the interpolator will be invoked with a value typically in the range [0, 1], where 0 represents the extreme negative value, 0.5 represents the neutral value, and 1 represents the extreme positive value. For example, using [d3.interpolateSpectral](https://github.com/d3/d3-scale-chromatic/blob/master/README.md#interpolateSpectral):
...
See [*continuous*.domain](README.md.md.md.md.md.md.md.md##continuous_domain). Note that a diverging scale’s domain must be numeric and must contain exactly three values. The default domain is [0, 0.5, 1].
...
See [*continuous*.range](README.md.md.md.md.md.md##continuous_rangeRound). If *range* is specified, implicitly uses [d3.interpolateRound](https://github.com/d3/d3-interpolate/blob/master/README.md#interpolateRound) as the interpolator.
...
See [*continuous*.unknown](README.md##continuous_unknown).
...
A [diverging scale](README.md.md.md.md##diverging-scales) with a logarithmic transform, analogous to a [log scale](README.md.md.md.md.md##log-scales).
...
A [diverging scale](README.md.md.md.md##diverging-scales) with an exponential transform, analogous to a [power scale](README.md.md##pow-scales).
...
A [diverging scale](README.md.md.md.md##diverging-scales) with a square-root transform, analogous to a [d3.scaleSqrt](README.md.md##scaleSqrt).
...
Given a *value* in the input [domain](README.md.md##quantize_domain), returns the corresponding value in the output [range](README.md.md##quantize_range). For example, to apply a color encoding:
...
Returns the extent of values in the [domain](README.md.md##quantize_domain) [<i>x0</i>, <i>x1</i>] for the corresponding *value* in the [range](README.md.md.md##quantize_range): the inverse of [*quantize*](README.md##_quantize). This method is useful for interaction, say to determine the value in the domain that corresponds to the pixel location under the mouse.
...
Equivalent to [*continuous*.ticks](README.md.md.md.md##continuous_ticks).
...
Equivalent to [*continuous*.tickFormat](README.md.md##continuous_tickFormat).
...
Equivalent to [*continuous*.nice](README.md##continuous_nice).
...
Returns the array of computed thresholds within the [domain](README.md.md##quantize_domain).
...
Constructs a new quantile scale with the specified [*domain*](README.md##quantile_domain) and [*range*](README.md##quantile_range). If either *domain* or *range* is not specified, each defaults to the empty array. The quantile scale is invalid until both a domain and range are specified.
...
Given a *value* in the input [domain](README.md.md##quantile_domain), returns the corresponding value in the output [range](README.md.md##quantile_range).
...
Returns the extent of values in the [domain](README.md.md##quantile_domain) [<i>x0</i>, <i>x1</i>] for the corresponding *value* in the [range](README.md.md.md##quantile_range): the inverse of [*quantile*](README.md##_quantile). This method is useful for interaction, say to determine the value in the domain that corresponds to the pixel location under the mouse.
...
Threshold scales are similar to [quantize scales](README.md.md.md##quantize-scales), except they allow you to map arbitrary subsets of the domain to discrete values in the range. The input domain is still continuous, and divided into slices based on a set of threshold values. See [this choropleth](https://observablehq.com/@d3/threshold-choropleth) for an example.
...
Given a *value* in the input [domain](README.md.md##threshold_domain), returns the corresponding value in the output [range](README.md.md##threshold_range). For example:
...
Returns the extent of values in the [domain](README.md.md##threshold_domain) [<i>x0</i>, <i>x1</i>] for the corresponding *value* in the [range](README.md.md.md##threshold_range), representing the inverse mapping from range to domain. This method is useful for interaction, say to determine the value in the domain that corresponds to the pixel location under the mouse. For example:
...
Unlike [continuous scales](README.md.md.md.md.md.md.md.md.md.md##continuous-scales), ordinal scales have a discrete domain and range. For example, an ordinal scale might map a set of named categories to a set of colors, or determine the horizontal positions of columns in a column chart.
...
Constructs a new ordinal scale with the specified [*domain*](README.md##ordinal_domain) and [*range*](README.md##ordinal_range). If *domain* is not specified, it defaults to the empty array. If *range* is not specified, it defaults to the empty array; an ordinal scale always returns undefined until a non-empty range is defined.
...
Given a *value* in the input [domain](README.md.md##ordinal_domain), returns the corresponding value in the output [range](README.md.md##ordinal_range). If the given *value* is not in the scale’s [domain](README.md.md##ordinal_domain), returns the [unknown](README.md##ordinal_unknown); or, if the unknown value is [implicit](README.md##scaleImplicit) (the default), then the *value* is implicitly added to the domain and the next-available value in the range is assigned to *value*, such that this and subsequent invocations of the scale given the same input *value* return the same output value.
...
Setting the domain on an ordinal scale is optional if the [unknown value](README.md.md##ordinal_unknown) is [implicit](README.md.md##scaleImplicit) (the default). In this case, the domain will be inferred implicitly from usage by assigning each unique value passed to the scale a new value from the range. Note that an explicit domain is recommended to ensure deterministic behavior, as inferring the domain from usage will be dependent on ordering.
...
If *value* is specified, sets the output value of the scale for unknown input values and returns this scale. If *value* is not specified, returns the current unknown value, which defaults to [implicit](README.md.md.md##scaleImplicit). The implicit value enables implicit domain construction; see [*ordinal*.domain](README.md.md.md.md##ordinal_domain).
...
A special value for [*ordinal*.unknown](README.md.md.md##ordinal_unknown) that enables implicit domain construction: unknown values are implicitly added to the domain.

#### Band Scales

Band scales are like [ordinal scales](README.md.md.md##ordinal-scales) except the output range is continuous and numeric. Discrete output values are automatically computed by the scale by dividing the continuous range into uniform bands. Band scales are typically used for bar charts with an ordinal or categorical dimension. The [unknown value](README.md.md##ordinal_unknown) of a band scale is effectively undefined: they do not allow implicit domain construction.
...
Constructs a new band scale with the specified [*domain*](README.md##band_domain) and [*range*](README.md##band_range), no [padding](README.md##band_padding), no [rounding](README.md##band_round) and center [alignment](README.md##band_align). If *domain* is not specified, it defaults to the empty domain. If *range* is not specified, it defaults to the unit range [0, 1].
...
Given a *value* in the input [domain](README.md.md##band_domain), returns the start of the corresponding band derived from the output [range](README.md.md##band_range). If the given *value* is not in the scale’s domain, returns undefined.
...
Sets the scale’s [*range*](README.md.md.md##band_range) to the specified two-element array of numbers while also enabling [rounding](README.md.md##band_round). This is a convenience method equivalent to:
...
If *padding* is specified, sets the inner padding to the specified number which must be less than or equal to 1. If *padding* is not specified, returns the current inner padding which defaults to 0. The inner padding specifies the proportion of the range that is reserved for blank space between bands; a value of 0 means no blank space between bands, and a value of 1 means a [bandwidth](README.md##band_bandwidth) of zero.
...
If *padding* is specified, sets the outer padding to the specified number which is typically in the range [0, 1]. If *padding* is not specified, returns the current outer padding which defaults to 0. The outer padding specifies the amount of blank space, in terms of multiples of the [step](README.md##band_step), to reserve before the first band and after the last band.
...
A convenience method for setting the [inner](README.md##band_paddingInner) and [outer](README.md##band_paddingOuter) padding to the same *padding* value. If *padding* is not specified, returns the inner padding.
...
Point scales are a variant of [band scales](README.md.md.md##band-scales) with the bandwidth fixed to zero. Point scales are typically used for scatterplots with an ordinal or categorical dimension. The [unknown value](README.md.md##ordinal_unknown) of a point scale is always undefined: they do not allow implicit domain construction.
...
Constructs a new point scale with the specified [*domain*](README.md##point_domain) and [*range*](README.md##point_range), no [padding](README.md##point_padding), no [rounding](README.md##point_round) and center [alignment](README.md##point_align). If *domain* is not specified, it defaults to the empty domain. If *range* is not specified, it defaults to the unit range [0, 1].
...
Given a *value* in the input [domain](README.md.md##point_domain), returns the corresponding point derived from the output [range](README.md.md##point_range). If the given *value* is not in the scale’s domain, returns undefined.
...
Sets the scale’s [*range*](README.md.md.md##point_range) to the specified two-element array of numbers while also enabling [rounding](README.md.md##point_round). This is a convenience method equivalent to:
...
If *padding* is specified, sets the outer padding to the specified number which is typically in the range [0, 1]. If *padding* is not specified, returns the current outer padding which defaults to 0. The outer padding specifies the amount of blank space, in terms of multiples of the [step](README.md##band_step), to reserve before the first point and after the last point. Equivalent to [*band*.paddingOuter](README.md.md##band_paddingOuter).
