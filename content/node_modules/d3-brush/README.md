...
Brushes also support programmatic control. For example, you can listen to [*end* events](README.md##brush-events), and then initiate a transition with [*brush*.move](README.md##brush_move) to snap the brush selection to semantic boundaries:
...
The overlay rect covers the brushable area defined by [*brush*.extent](README.md##brush_extent). The selection rect covers the area defined by the current [brush selection](README.md##brushSelection). The handle rects cover the edges and corners of the brush selection, allowing the corresponding value in the brush selection to be modified interactively. To modify the brush selection programmatically, use [*brush*.move](README.md.md##brush_move).
...
An alias for [*brush*.move](README.md.md.md##brush_move) with the null selection.
...
Touch event listeners are only registered if the detector returns truthy for the corresponding element when the brush is [applied](README.md##_brush). The default detector works well for most browsers that are capable of touch input, but not all; Chrome’s mobile device emulator, for example, fails detection.
...
If *size* is specified, sets the size of the brush handles to the specified number and returns the brush. If *size* is not specified, returns the current handle size, which defaults to six. This method must be called before [applying the brush](README.md.md##_brush) to a selection; changing the handle size does not affect brushes that were previously rendered.
...
See [*dispatch*.on](https://github.com/d3/d3-dispatch#dispatch_on) and [Brush Events](README.md.md##brush-events) for more.
...
Returns the current brush selection for the specified *node*. Internally, an element’s brush state is stored as *element*.\_\_brush; however, you should use this method rather than accessing it directly. If the given *node* has no selection, returns null. Otherwise, the *selection* is defined as an array of numbers. For a [two-dimensional brush](README.md.md##brush), it is [[*x0*, *y0*], [*x1*, *y1*|*x*-brush]], where *x0* is the minimum *x*-value, *y0* is the minimum *y*-value, *x1* is the maximum *x*-value, and *y1* is the maximum *y*-value. For an [*x*-brush](README.md.md##brushX), it is [*x0*, *x1*]; for a [*y*-brush](README.md.md##brushY), it is [*y0*, *y1*].

### Brush Events

When a [brush event listener](README.md##brush_on) is invoked, it receives the current brush event. The *event* object exposes several fields:

* `target` - the associated [brush behavior](README.md.md.md##brush).
* `type` - the string “start”, “brush” or “end”; see [*brush*.on](README.md.md##brush_on).
* `selection` - the current [brush selection](README.md.md##brushSelection).
