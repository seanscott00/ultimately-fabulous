...
- [Install and use](README.md##%EF%B8%8F-install-and-use)
- [Examples](README.md##-examples)
- [Context](README.md##-context)
- [Extended regex syntax](README.md##-extended-regex-syntax)
  - [Atomic groups](README.md##atomic-groups)
  - [Possessive quantifiers](README.md##possessive-quantifiers)
  - [Subroutines](README.md##subroutines)
  - [Subroutine definition groups](README.md##subroutine-definition-groups)
  - [Recursion](README.md##recursion)
- [Flags](README.md##-flags)
  - [Implicit flags](README.md##implicit-flags)
  - [Flag <kbd>v</kbd>](README.md##flag-v)
  - [Flag <kbd>x</kbd>](README.md##flag-x)
  - [Flag <kbd>n</kbd>](README.md##flag-n)
- [Interpolation](README.md##-interpolation)
  - [`RegExp` instances](#interpolating-regexes)
  - [Escaped strings](README.md##interpolating-escaped-strings)
  - [Partial patterns](README.md##interpolating-partial-patterns)
  - [Interpolation principles](README.md##interpolation-principles)
  - [Interpolation contexts](README.md##interpolation-contexts)
- [Options](README.md##-options)
  - [Returning a string](README.md##returning-a-string)
...
> Possessive quantifiers are syntactic sugar for [atomic groups](README.md.md##atomic-groups) when their contents are a single repeated item (which could be a token, character class, or group).
...
Subroutines go beyond the composition benefits of [interpolation](README.md.md##-interpolation). Apart from the obvious difference that they don't require variables to be defined outside of the regex, they also don't simply insert the referenced subpattern.
...
- Although subroutines can be chained to any depth, a descriptive error is thrown if they're used recursively. Support for recursion can be added via a plugin (see [*Recursion*](README.md.md##recursion)).
...
The syntax `(?(DEFINE)…)` can be used at the end of a regex to define subpatterns for use by reference only. When combined with [subroutines](README.md.md##subroutines), this enables writing regexes in a grammatical way that can significantly improve readability and maintainability.
...
`RegExp` instances interpolated into the pattern preserve their own flags locally (see [*Interpolating regexes*](README.md.md##interpolating-regexes)).
...
> For special situations such as when using Regex+ within other tools, implicit flags can be disabled. See: [*Options*](README.md.md##-options).
...
- Regex+ is not primarily a backward compatibility library, so in order to remain lightweight, it doesn't transpile flag <kbd>v</kbd>'s new features out of the box. By replacing the default function, you can add backward compatible support for these features. See also: [*Compatibility*](README.md.md##-compatibility).
...
- **`x`** — Disables implicit, emulated [flag <kbd>x</kbd>](README.md.md##flag-x).
- **`n`** — Disables implicit, emulated [flag <kbd>n</kbd>](README.md.md##flag-n). Note that, although it's safe to use unnamed captures and numbered backreferences within a regex when flag <kbd>n</kbd> is disabled, referencing submatches by number from *outside* a regex (e.g. in replacement strings) can result in incorrect values because extended syntax (atomic groups and subroutines) might add "emulation groups" to generated regex source. It's therefore recommended to enable the `subclass` option when disabling `n`.
- **`v`** — Disables implicit [flag <kbd>v</kbd>](README.md.md##flag-v) even when it's supported natively, resulting in flag <kbd>u</kbd> being added instead (in combination with the `unicodeSetsPlugin`).
- **`atomic`** — Disables [atomic groups](README.md.md##atomic-groups) and [possessive quantifiers](README.md.md##possessive-quantifiers), resulting in a syntax error if they're used.
- **`subroutines`** — Disables [subroutines](README.md.md##subroutines) and [subroutine definition groups](README.md.md.md##subroutine-definition-groups), resulting in a syntax error if they're used.
...
For regexes that rely on or have the potential to trigger heavy backtracking, you can dramatically improve beyond native performance via `regex`'s [atomic groups](README.md.md##atomic-groups) and [possessive quantifiers](README.md.md##possessive-quantifiers).
