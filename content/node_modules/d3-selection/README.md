...
Selections allow powerful data-driven transformation of the document object model (DOM): set [attributes](README.md##selection_attr), [styles](README.md##selection_style), [properties](README.md##selection_property), [HTML](README.md##selection_html) or [text](README.md##selection_text) content, and more. Using the [data join](README.md##joining-data)’s [enter](README.md##selection_enter) and [exit](README.md.md##selection_enter) selections, you can also [add](README.md##selection_append) or [remove](README.md##selection_remove) elements to correspond to data.
...
* [Selecting Elements](README.md##selecting-elements)
* [Modifying Elements](README.md##modifying-elements)
* [Joining Data](README.md.md##joining-data)
* [Handling Events](README.md##handling-events)
* [Control Flow](README.md##control-flow)
* [Local Variables](README.md##local-variables)
* [Namespaces](README.md##namespaces)

### Selecting Elements

Selection methods accept [W3C selector strings](http://www.w3.org/TR/selectors-api/) such as `.fancy` to select elements with the class *fancy*, or `div` to select DIV elements. Selection methods come in two forms: select and selectAll: the former selects only the first matching element, while the latter selects all matching elements in document order. The top-level selection methods, [d3.select](#select) and [d3.selectAll](README.md.md##selectAll), query the entire document; the subselection methods, [*selection*.select](README.md.md##selection_select) and [*selection*.selectAll](README.md.md##selection_selectAll), restrict selection to descendants of the selected elements.
...
[Selects](README.md.md##select) the root element, `document.documentElement`. This function can also be used to test for selections (`instanceof d3.selection`) or to extend the selection prototype. For example, to add a method to check checkboxes:
...
Unlike [*selection*.selectAll](README.md.md##selection_selectAll), *selection*.select does not affect grouping: it preserves the existing group structure and indexes, and propagates data (if any) to selected children. Grouping plays an important role in the [data join](README.md.md.md##joining-data). See [Nested Selections](http://bost.ocks.org/mike/nest/) and [How Selections Work](http://bost.ocks.org/mike/selection/) for more on this topic.
...
Unlike [*selection*.select](README.md.md##selection_select), *selection*.selectAll does affect grouping: each selected descendant is grouped by the parent element in the originating selection. Grouping plays an important role in the [data join](README.md.md.md.md##joining-data). See [Nested Selections](http://bost.ocks.org/mike/nest/) and [How Selections Work](http://bost.ocks.org/mike/selection/) for more on this topic.
...
This is approximately equivalent to using [d3.selectAll](README.md.md##selectAll) directly, although the indexes may be different:
...
Or using [*selection*.select](README.md.md##selection_select) (and avoiding an arrow function, since *this* is needed to refer to the current element):
...
This method is used internally by [*selection*.join](README.md##selection_join) to merge the [enter](README.md.md.md##selection_enter) and [update](README.md.md##selection_data) selections after [binding data](README.md.md.md.md.md##joining-data). You can also merge explicitly, although note that since merging is based on element index, you should use operations that preserve index, such as [*selection*.select](README.md.md##selection_select) instead of [*selection*.filter](README.md##selection_filter). For example:
...
See [*selection*.data](README.md##selection_data) for more.
...
Given the specified *selector*, returns a function which returns true if `this` element [matches](https://developer.mozilla.org/en-US/docs/Web/API/Element/matches) the specified selector. This method is used internally by [*selection*.filter](README.md.md##selection_filter). For example, this:
...
Given the specified *selector*, returns a function which returns the first descendant of `this` element that matches the specified selector. This method is used internally by [*selection*.select](README.md.md##selection_select). For example, this:
...
Given the specified *selector*, returns a function which returns all descendants of `this` element that match the specified selector. This method is used internally by [*selection*.selectAll](README.md.md##selection_selectAll). For example, this:
...
Returns the value of the style property with the specified *name* for the specified *node*. If the *node* has an inline style with the specified *name*, its value is returned; otherwise, the [computed property value](https://developer.mozilla.org/en-US/docs/Web/CSS/computed_value) is returned. See also [*selection*.style](README.md.md##selection_style).
...
The specified *name* may have a namespace prefix, such as `xlink:href` to specify the `href` attribute in the XLink namespace. See [namespaces](README.md.md##namespaces) for the map of supported namespaces; additional namespaces can be registered by adding to the map.
...
If the specified *type* is a string, appends a new element of this type (tag name) as the last child of each selected element, or before the next following sibling in the update selection if this is an [enter selection](README.md.md.md.md##selection_enter). The latter behavior for enter selections allows you to insert elements into the DOM in an order consistent with the new bound data; however, note that [*selection*.order](README.md##selection_order) may still be required if updating elements change order (*i.e.*, if the order of new data is inconsistent with old data).
...
In both cases, this method returns a new selection containing the appended elements. Each new element inherits the data of the current elements, if any, in the same manner as [*selection*.select](README.md.md##selection_select).

The specified *name* may have a namespace prefix, such as `svg:text` to specify a `text` attribute in the SVG namespace. See [namespaces](README.md.md##namespaces) for the map of supported namespaces; additional namespaces can be registered by adding to the map. If no namespace is specified, the namespace will be inherited from the parent element; or, if the name is one of the known prefixes, the corresponding namespace will be used (for example, `svg` implies `svg:svg`).
...
Removes the selected elements from the document. Returns this selection (the removed elements) which are now detached from the DOM. There is not currently a dedicated API to add removed elements back to the document; however, you can pass a function to [*selection*.append](README.md.md##selection_append) or [*selection*.insert](README.md##selection_insert) to re-add elements.
...
Returns a new selection that contains a copy of each group in this selection sorted according to the *compare* function. After sorting, re-inserts elements to match the resulting order (per [*selection*.order](README.md.md##selection_order)).
...
Re-inserts elements into the document such that the document order of each group matches the selection order. This is equivalent to calling [*selection*.sort](README.md##selection_sort) if the data is already sorted, but much faster.
...
Given the specified element *name*, returns a function which creates an element of the given name, assuming that `this` is the parent element. This method is used internally by [*selection*.append](README.md.md##selection_append) and [*selection*.insert](README.md.md.md##selection_insert) to create new elements. For example, this:
...
See [namespace](README.md##namespace) for details on supported namespace prefixes, such as for SVG elements.
...
The *update* and *enter* selections are returned in data order, while the *exit* selection preserves the selection order prior to the join. If a key function is specified, the order of elements in the selection may not match their order in the document; use [*selection*.order](README.md.md##selection_order) or [*selection*.sort](README.md##selection_sort) as needed. For more on how the key function affects the join, see [A Bar Chart, Part 2](http://bost.ocks.org/mike/bar/2/) and [Object Constancy](http://bost.ocks.org/mike/constancy/).

If *data* is not specified, this method returns the array of data for the selected elements.

This method cannot be used to clear bound data; use [*selection*.datum](README.md##selection_datum) instead.
...
Appends, removes and reorders elements as necessary to match the data that was previously bound by [*selection*.data](README.md##selection_data), returning the [merged](README.md##selection_merge) enter and update selection. This method is a convenient alternative to the explicit [general update pattern](https://bl.ocks.org/mbostock/3808218), replacing [*selection*.enter](README.md.md.md.md.md.md.md##selection_enter), [*selection*.exit](README.md.md.md##selection_exit), [*selection*.append](README.md.md##selection_append), [*selection*.remove](README.md.md.md##selection_remove), and [*selection*.order](README.md.md##selection_order). For example:
...
The *enter* function may be specified as a string shorthand, as above, which is equivalent to [*selection*.append](README.md.md##selection_append) with the given element name. Likewise, optional *update* and *exit* functions may be specified, which default to the identity function and calling [*selection*.remove](README.md.md.md.md##selection_remove), respectively. The shorthand above is thus equivalent to:
