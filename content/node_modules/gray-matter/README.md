...
* **flexible**: By default, gray-matter is capable of parsing [YAML](https://github.com/nodeca/js-yaml), [JSON](http://en.wikipedia.org/wiki/Json) and JavaScript front-matter. But other [engines](README.md##optionsengines) may be added.
* **extensible**: Use [custom delimiters](README.md##optionsdelimiters), or add support for [any language](README.md.md##optionsengines), like [TOML](http://github.com/mojombo/toml), [CoffeeScript](http://coffeescript.org), or [CSON](https://github.com/bevry/cson)
...
Pass a string and [options](README.md##options) to gray-matter:
...
* `file.excerpt` **{String}**: an excerpt, if [defined on the options](README.md##optionsexcerpt)
...
* `file.stringify` **{Function}**: [stringify](README.md##stringify) the file by converting `file.data` to a string in the given language, wrapping it in delimiters and prepending it to `file.content`.
...
Takes a string or object with `content` property, extracts and parses front-matter from the string, then returns an object with `data`, `content` and other [useful properties](README.md##returned-object).
...
Stringify an object to YAML or the specified language, and append it to the given string. By default, only YAML and JSON can be stringified. See the [engines](README.md##engines) section to learn how to stringify other languages.
...
* `options` **{Object}**: [Options](README.md.md##options) to pass to gray-matter and [js-yaml](https://github.com/nodeca/js-yaml).
...
Synchronously read a file from the file system and parse front matter. Returns the same object as the [main function](README.md##matter).

**Params**

* `filepath` **{String}**: file path of the file to read.
* `options` **{Object}**: [Options](README.md.md##options) to pass to gray-matter.
* `returns` **{Object}**: Returns [an object](README.md.md##returned-object) with `data` and `content`
...
Decrecated, please use [options.language](README.md##optionslanguage) instead.

### options.delims

Decrecated, please use [options.delimiters](README.md.md##optionsdelimiters) instead.

### options.parsers

Decrecated, please use [options.engines](README.md.md.md##optionsengines) instead.
