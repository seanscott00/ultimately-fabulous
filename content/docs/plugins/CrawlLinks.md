...
This plugin parses links and processes them to point to the right places. It is also needed for embedded links (like images). See [[../features/Obsidian Compatibility|Obsidian Compatibility]] for more information.
...
- `markdownLinkResolution`: Sets the strategy for resolving Markdown paths, can be `"absolute"` (default), `"relative"` or `"shortest"`. You should use the same setting here as in [[../features/Obsidian Compatibility|Obsidian]].
